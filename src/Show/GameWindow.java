...
public abstract class GameWindow{
	...
	public static void create(int width, int height, String title, int numberOfBufferStrategy, boolean tankServer){
		if(created) return;
		...
		store=new JMenuItem(new AbstractAction("Store and Exit") {
		    public void actionPerformed(ActionEvent e) {
		    	Main.game.getTank().setIsStore(true);
		    }
		});
		menu.add(store);
		
		if(tankServer){
		stored=new JMenuItem(new AbstractAction("History") {
		    public void actionPerformed(ActionEvent e){
		    	GameWindow.window.setVisible(false);
		    	Main.menuFrame.setVisible(true);
		    	Main.menuFrame.load.setVisible(true);
		    }
		});
		menu.add(stored);
		}
